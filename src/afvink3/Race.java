...
public class Race extends JFrame implements ActionListener {
	...
	public static final int lengte = 250;
	...
	ArrayList<Image> pokemon = new ArrayList<Image>();
    ...
    private void startRace(Graphics g) {
         ...
         * kunt initialiseren.
         */
        h1 = new Paard("Henk", Color.DARK_GRAY);
        h2 = new Paard("Ragna", Color.PINK);
        h3 = new Paard("kaHuub", Color.BLUE);
        h4 = new Paard("Meme-meister", new Color(196,200,39));
        h5 = new Paard("Thijs", Color.CYAN);
    ...
    private void createGUI() {
    	...
    	/*
      		try {
    		paardImage = ImageIO.read(new File("help/snor.png"));
    		paardImage = paardImage.getScaledInstance(40, 40, Image.SCALE_SMOOTH);
    	...
    	*/
    	...
    	try {
			BufferedImage alles = ImageIO.read(new File("resources/pokemon.png"));


			for(int i = 0; i < 18;i++) {
				for(int t = 0; t < 28; t++) {
					
					Image tempImage = alles.getSubimage(80*t, 80*i, 80, 80);
					//tempImage = tempImage.getScaledInstance(40, 40, Image.SCALE_SMOOTH);
					pokemon.add(tempImage);
    ...
    private void tekenPaard(Graphics g, Paard h) {
        ...
        Image plaatje = null;
        System.out.println(h.getNaam());
        if(h.getNaam() == "Ragna") {
        	plaatje = pokemon.get(321-1);
        	//h.run();
        }
        else if(h.getNaam() == "Henk") {
        	plaatje = pokemon.get(151-1);
        	
        }
        else if(h.getNaam() == "Thijs") {
        	plaatje = pokemon.get(202-1);
        }
        else if(h.getNaam() == "kaHuub") {
        	plaatje = pokemon.get(108-1);
        }
        else if(h.getNaam() == "Meme-meister") {
        	plaatje = pokemon.get(122-1);
        }
        
        
        
        else {
        	plaatje = pokemon.get(h.getPaardNummer());
        }
        
       g.drawImage(plaatje, h.getAfstand()-40, 80 * h.getPaardNummer() ,null );
    }
